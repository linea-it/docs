O LIneA oferece um conjunto de serviços e plataformas que dão suporte às atividades científicas dos membros da colaboração **LSST (Legacy Survey of Space and Time)**. Esses recursos incluem ambientes de análise, ferramentas especializadas e acesso a dados sob embargo.

Para garantir a segurança e a correta autorização de acesso, **apenas usuários que fazem parte oficialmente da colaboração LSST** podem utilizar esses recursos. Por esse motivo, o processo de **registro no LIneA** exige que o usuário, em uma das etapas, realize o **login com uma conta ativa do Rubin Science Platform (RSP)** por meio da opção **“LSST Members (RSP Account)”**.

#### Importante: Ambientes Distintos

Apesar de existir essa etapa de autenticação, é fundamental destacar que:

- **O LIneA e o RSP são ambientes totalmente independentes**, administrados por equipes diferentes e hospedados em infraestruturas distintas.

- O LIneA **não possui qualquer autonomia** para criar, gerenciar ou recuperar contas do RSP.

- **Conseguir acessar o RSP não garante automaticamente acesso aos recursos oferecidos pelo LIneA.** O login via RSP é apenas uma das etapas obrigatórias do nosso processo de verificação.

Caso o usuário **não possua uma conta no RSP**, **o procedimento de registro no LIneA não poderá ser concluído**. Nesses casos, recomendamos que o usuário primeiro solicite sua conta no RSP.

- Para criar um conta no RSP, acesse:  https://rsp.lsst.io/index.html

--- 

Siga as instruções para habilitar os recursos oferecidos pela LIneA aos membros do LSST.

!!! warning "Mesmo usuários já registrados no LIneA como público geral devem realizar todas as etapas a seguir."


**1.** Acesse: [https://scienceplatform.linea.org.br/lsp](https://scienceplatform.linea.org.br/lsp)

**2.** Clique em **"Sign In"** (canto superior direito da tela).

**3.** Clique no botão de acesso via RSP: **"LSST MEMBERS (RSP ACCOUNT)"**.

**4.** Na página do CILogon, procure e selecione sua instituição no campo "Select an Identity Provider".

**5.** Após a etapa anterior, será exibida uma tela com a mensagem **"Unauthorized Access"**. Neste momento, clique em **"register here"**.

**6.** Preencha e submeta o formulário de registro exibido e aceite os termos.

Siga este procedimento através do vídeo tutorial disponível em: **<ins>https://youtu.be/nyRN5xVRRqo</ins>**.

Em caso de dúvidas, entre em contato com o Service Desk pelo e-mail [helpdesk@linea.org.br](mailto:helpdesk@linea.org.br).
